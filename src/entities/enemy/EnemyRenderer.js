...
class EnemyRenderer {
    ...
    static render(enemy, ctx) {
        ...
        // [POLYBIUS VIBE] 3D Wireframe Geometry using cached sprites
        // Each enemy has unique rotation based on its ID and time
        const shapeCache = window.Game?.enemyShapeCache;

        if (shapeCache) {
            // Calculate rotation based on enemy ID (variation) + time (animation)
            // Use a slower rotation speed for retro feel
            const idHash = parseInt(enemy.id || '0', 36) % 100;
            const rotSpeed = enemy.isBoss ? 0.3 : (enemy.isElite ? 0.5 : 0.7);
            const timeBase = Date.now() * 0.001 * rotSpeed;

            // Different rotation offsets per enemy for organic swarm look
            const rotX = timeBase + idHash * 0.1;
            const rotY = timeBase * 0.7 + idHash * 0.15;

            // Get enemy type for shape selection
            const enemyType = enemy.isBoss ? 'boss' : (enemy.enemyType || 'basic');

            // Get cached sprite
            const sprite = shapeCache.getSprite(enemyType, drawRadius, rotX, rotY, fillColor);

            if (sprite) {
                ctx.save();
                ctx.globalAlpha = combinedAlpha;

                // Draw the cached wireframe sprite
                ctx.drawImage(
                    sprite.canvas,
                    enemy.x - sprite.halfSize,
                    enemy.y - sprite.halfSize
                );

                // Inner core glow (smaller, central energy)
                ctx.globalAlpha = combinedAlpha * 0.5;
                ctx.fillStyle = fillColor;
                ctx.beginPath();
                ctx.arc(enemy.x, enemy.y, drawRadius * 0.25, 0, Math.PI * 2);
                ctx.fill();

                ctx.restore();
            } else {
                // Fallback to simple shape if cache miss
                this._renderFallbackShape(enemy, ctx, drawRadius, fillColor, combinedAlpha);
            }
        } else {
            // Fallback if shape cache not available
            this._renderFallbackShape(enemy, ctx, drawRadius, fillColor, combinedAlpha);
        }
    ...
    static renderBatch(enemies, ctx) {
        ...
        for (const [key, batch] of bodyBatches) {
            ...
            if (shapeCache) {
                // [3D WIREFRAME] Draw each enemy using cached sprites
                ...
                for (let i = 0; i < batch.length; i++) {
                    ...
                    // Calculate rotation (same logic as single render)
                    const idHash = parseInt(enemy.id || '0', 36) % 100;
                    const rotSpeed = enemy.isBoss ? 0.3 : (enemy.isElite ? 0.5 : 0.7);
                    const timeBase = Date.now() * 0.001 * rotSpeed;
                    const rotX = timeBase + idHash * 0.1;
                ...
                // Pass 2: Inner cores (batched as circles)
                ctx.globalAlpha = batchAlpha * 0.5;
                ctx.fillStyle = fillColor;
                ctx.beginPath();
                for (let i = 0; i < batch.length; i++) {
                    const enemy = batch[i];
                    const pulseScale = (enemy.isElite || enemy.isBoss) ? (enemy.pulseIntensity || 1.0) : 1.0;
                    const r = enemy.radius * pulseScale * 0.25;
                    ctx.moveTo(enemy.x + r, enemy.y);
                    ctx.arc(enemy.x, enemy.y, r, 0, Math.PI * 2);
                }
                ctx.fill();
            } else {
                // Fallback: Original 2D batched shapes
                ctx.globalAlpha = batchAlpha;
    ...
    /**
     * Fallback shape rendering when 3D cache isn't available
     * Uses simple 2D shapes (original behavior)
     */
    static _renderFallbackShape(enemy, ctx, drawRadius, fillColor, combinedAlpha) {
        ctx.save();
        ctx.translate(enemy.x, enemy.y);

        // Simple rotation for basic animation
        const spin = (Date.now() * 0.001) + (parseInt(enemy.id || '0', 36) % 10);
        ...
        if (enemy.isBoss) {
            ...
            for (let i = 0; i < 6; i++) {
                ...
                if (i === 0) ctx.moveTo(Math.cos(angle) * drawRadius, Math.sin(angle) * drawRadius);
                else ctx.lineTo(Math.cos(angle) * drawRadius, Math.sin(angle) * drawRadius);
            }
        } else if (enemy.isElite) {
            // Diamond
