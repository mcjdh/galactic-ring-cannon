class GameManager {
    constructor() {
        ...
        // Critical XP chance
        this.critXpChance = (window.GAME_CONSTANTS?.META?.CRIT_XP_CHANCE) || 0.15;
    ...
    createExplosion(x, y, radius, color) {
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            if (this.particlePool && this.particlePool.length > 0) {
                ...
                // 🤖 RESONANT NOTE: Prefer pooled particles over direct instantiation
                if (window.optimizedParticles) {
                    const motion = MathUtils.randomParticleMotion();
                    window.optimizedParticles.spawnParticle({
                        x, y, 
                        vx: Math.cos(motion.angle) * motion.speed,
                        vy: Math.sin(motion.angle) * motion.speed,
                        size: 2 + Math.random() * 6,
                        color: color || '#ff6b35',
                        life: 0.5 + Math.random() * 0.5,
                        type: 'explosion'
                    });
                    continue; // Skip manual particle management
                }
    ...
    createLevelUpEffect(x, y) {
        ...
        try {
            ...
            // Use logger for better error handling instead of console.warn
            (window.logger?.warn || (() => {}))('Failed to create ShockwaveParticle:', error);
        ...
        switch (type) {
            case 'lightning': {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    // 🤖 RESONANT NOTE: Lightning effect using pooled particles
                    if (window.optimizedParticles) {
                        window.optimizedParticles.spawnParticle({
                        ...
                        const particle = new Particle(
                            ...
                            2 + Math.random() * 3, color || '#3498db', 0.2 + Math.random() * 0.2
                        );
                        this.tryAddParticle(particle);
                    }
                }
                // Create central flash using pooled system
                if (window.optimizedParticles) {
                    window.optimizedParticles.spawnParticle({
                        x, y, vx: 0, vy: 0, size: size * 0.5,
                        color: color || '#3498db', life: 0.15, type: 'flash'
                    });
                } else {
                    const flash = new Particle(x, y, 0, 0, size * 0.5, color || '#3498db', 0.15);
            ...
            case 'ricochet': {
                ...
                for (let i = 0; i < count; i++) {
                    ...
                    // 🤖 RESONANT NOTE: Ricochet effect using pooled system
                    if (window.optimizedParticles) {
                        window.optimizedParticles.spawnParticle({
                            x, y, vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
                            size: 2 + Math.random() * 3, color: color || '#f39c12',
                            life: 0.2 + Math.random() * 0.2, type: 'ricochet'
                        });
                    } else {
                        const particle = new Particle(
                            x, y, Math.cos(angle) * speed, Math.sin(angle) * speed,
                            2 + Math.random() * 3, color || '#f39c12', 0.2 + Math.random() * 0.2
                        );
                        this.tryAddParticle(particle);
                    }
                    this.tryAddParticle(particle);
    ...
    activateBossMode() {
        try {
            ...
            // Use logger instead of console.error for better error handling
            (window.logger?.error || (() => {}))('Error in activateBossMode:', error);
            // Graceful degradation - continue without boss mode features
        }
    }
