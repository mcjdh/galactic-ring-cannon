...
class Player {
    constructor(x, y) {
        ...
        // Using GameConstants for all gameplay values
        // 🤖 RESONANT NOTE: Magic numbers migrated to GameConstants.js for consistency
        this.speed = window.GAME_CONSTANTS?.PLAYER.BASE_SPEED || 220;
        this.health = window.GAME_CONSTANTS?.PLAYER.BASE_HEALTH || 120;
        this.maxHealth = window.GAME_CONSTANTS?.PLAYER.BASE_HEALTH || 120;
        this.xp = 0;
        this.xpToNextLevel = window.GAME_CONSTANTS?.PLAYER.INITIAL_XP_TO_LEVEL || 212;
        this.level = 1;
        this.isDead = false;
    ...
    createTrailParticle(x, y) {
        ...
        // 🤖 RESONANT NOTE: Converted to pooled particle system for better performance
        if (window.optimizedParticles) {
            window.optimizedParticles.spawnParticle({
                x, y, vx: 0, vy: 0, size: trailSize, 
                color: this.color, life: duration, type: 'trail'
            });
        } else if (gameManager.tryAddParticle) {
            const particle = new Particle(x, y, 0, 0, trailSize, this.color, duration);
    ...
    createAOEEffect() {
        ...
        const baseCount = 24;
        const particleCount = MathUtils.budget(baseCount, factor, gm.maxParticles || 150, gm.particles?.length || 0);
        ...
        for (let i = 0; i < particleCount; i++) {
            ...
            // 🤖 RESONANT NOTE: Using pooled particles instead of direct instantiation
            if (window.optimizedParticles) {
                window.optimizedParticles.spawnParticle({
                    x: this.x, y: this.y,
                    vx: Math.cos(angle) * 300, vy: Math.sin(angle) * 300,
                    size: 3 + Math.random() * 3, color: '#3498db',
                0.3
            );
    ...
    createUpgradeStackEffect() {
        ...
        for (let i = 0; i < count; i++) {
            ...
            // 🤖 RESONANT NOTE: Converted to pooled system to reduce GC pressure
            if (window.optimizedParticles) {
                window.optimizedParticles.spawnParticle({
                    x: this.x + Math.cos(angle) * distance,
                    y: this.y + Math.sin(angle) * distance,
                    vx: Math.cos(angle) * 60, vy: Math.sin(angle) * 60,
                    size: 4, color: '#e67e22', life: 0.5, type: 'levelup'
            );
    ...
    doDodge() {
        ...
        if (gm && !gm.lowQuality) {
            const factor = (gm.particleReductionFactor || 1.0);
            const baseCount = 10;
            const count = MathUtils.budget(baseCount, factor, gm.maxParticles || 150, gm.particles?.length || 0);
            for (let i = 0; i < count; i++) {
            // 🤖 RESONANT NOTE: Using pooled particles for dodge effect
            if (window.optimizedParticles) {
                window.optimizedParticles.spawnParticle({
                    x: this.x, 
                this.y,
    ...
    createLightningEffect(from, to) {
        ...
        for (let i = 0, n = Math.max(0, Math.floor(5 * factor)); i < n; i++) {
            ...
            // 🤖 RESONANT NOTE: Spark effect using pooled system
            if (window.optimizedParticles) {
                window.optimizedParticles.spawnParticle({
                    x: baseX, y: baseY,
                    vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
                    size, color: '#81ecec', life: 0.15, type: 'spark'
                });
            } else {
                const sparkParticle = new Particle(
                    baseX, baseY, Math.cos(angle) * speed, Math.sin(angle) * speed,
                    size, '#81ecec', 0.15
                );
                if (gameManager?.tryAddParticle) gameManager.tryAddParticle(sparkParticle);
            }
    ...
    createRicochetEffect(fromX, fromY, toX, toY) {
        ...
        const particleCount = Math.max(0, Math.floor(MathUtils.clamp(baseParticles, 0, 15) * factor));
        for (let i = 0; i < particleCount; i++) {
    ...
    // 🚨 REMOVED DUPLICATE: This method was identical to doDodge() above
    // 🤖 RESONANT NOTE: All dodge functionality consolidated into doDodge() method
    // If you need dodge functionality, use: player.doDodge()
}
}